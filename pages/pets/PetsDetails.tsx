...
export default function PetDetails() {
  ...
  const servicoList = Array.from(
    new Set(
      (agendamentos ?? []).flatMap(
        (a) => a?.servicos?.map((s) => s.nome) ?? [],
      ),
    ),
  );
  ...
  const hasFilters = !!statusFilter || !!servicoFilter || !!dataDe || !!dataAte;
